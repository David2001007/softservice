...
function AgendaPage() {
  ...
  const filteredEventos = eventos.filter((e) =>
    (!filtroTecnico || e.tecnico.toLowerCase().includes(filtroTecnico.toLowerCase())) &&
    (!filtroTipo || e.tipoServico.toLowerCase().includes(filtroTipo.toLowerCase())) &&
      ...
      <div className="bg-surface border border-border rounded-xl p-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <div className="space-y-1">
            ...
            <input value={filtroTecnico} onChange={(e) => setFiltroTecnico(e.target.value)} placeholder="Filtrar por técnico..." className="w-full h-9 px-3 rounded-lg bg-background border border-border text-text text-sm placeholder-text-muted focus:outline-none focus:border-primary transition-colors" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-text-muted">Tipo de Serviço</label>
            <input value={filtroTipo} onChange={(e) => setFiltroTipo(e.target.value)} placeholder="Instalação, Manutenção..." className="w-full h-9 px-3 rounded-lg bg-background border border-border text-text text-sm placeholder-text-muted focus:outline-none focus:border-primary transition-colors" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-medium text-text-muted">Status</label>
            <select value={filtroStatus} onChange={(e) => setFiltroStatus(e.target.value)} className="w-full h-9 px-3 rounded-lg bg-background border border-border text-text text-sm focus:outline-none focus:border-primary transition-colors cursor-pointer">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <div className="xl:col-span-2 bg-surface border border-border rounded-xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-muted hover:text-text hover:bg-surface-hover transition-colors">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <h2 className="text-sm font-semibold text-text capitalize">
              {format(currentMonth, 'MMMM yyyy', { locale: ptBR })}
            </h2>
            <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="w-8 h-8 rounded-lg flex items-center justify-center text-text-muted hover:text-text hover:bg-surface-hover transition-colors">
          ...
          <div className="grid grid-cols-7 border-b border-border">
            {['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb'].map((d) => (
              <div key={d} className="py-2 text-center text-xs font-semibold text-text-muted">{d}</div>
            ))}
          </div>
                ...
                >
                  ...
                  <div className="space-y-0.5">
                    {evs.slice(0, 2).map((ev) => (
                      <div key={ev.id} className={`text-[10px] px-1.5 py-0.5 rounded border truncate leading-tight font-medium ${statusColors[ev.status] ?? 'bg-muted border-border text-text-muted'}`}>
                        {ev.numero}
                      </div>
                    ))}
                    {evs.length > 2 && (
                      <div className="text-[10px] text-text-muted pl-1">+{evs.length - 2} mais</div>
                    )}
                  </div>
                </button>
              )
        ...
        <div className="bg-surface border border-border rounded-xl overflow-hidden">
          ...
          <div className="p-4 space-y-3 max-h-[520px] overflow-y-auto">
            {selectedDayEventos.length === 0 ? (
              <p className="text-sm text-text-muted text-center py-6">Nenhum atendimento agendado para este dia.</p>
            ) : (
              selectedDayEventos.map((ev) => (
                <Link key={ev.id} to="/ordens-servico/$id/gerenciar" params={{ id: String(ev.osId) }}>
                  <div className="p-3 rounded-xl bg-background border border-border hover:border-primary/40 transition-colors cursor-pointer space-y-2">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-xs font-bold text-gold">{ev.numero}</span>
                      <StatusBadge value={ev.status} type="os" />
                    </div>
                    <p className="text-sm font-medium text-text">{ev.cliente}</p>
                    <p className="text-xs text-text-muted">{ev.tipoServico}</p>
                    <div className="flex items-center justify-between text-xs text-text-muted">
                      <span>{ev.tecnico}</span>
                      <span className="font-semibold text-text">{formatDate(ev.dataAgendada, { time: true }).split(' ')[1]}</span>
                    </div>
                  </div>
                </Link>
              ))
